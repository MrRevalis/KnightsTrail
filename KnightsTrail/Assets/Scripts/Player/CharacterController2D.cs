...
public class CharacterController2D : MonoBehaviour
{
	...
	public bool moving = false; // For determining which way the player is currently facing.
	...
	public void Move(float move, float climb, bool crouch, bool jump, bool isClimbing)
	{
		...
		if (m_Grounded && jump)
		{
			// If the character has a ceiling preventing them from standing up (so jumping is impossible), keep them crouching
			if (Physics2D.OverlapCircle(m_CeilingCheck.position, k_CeilingRadius, m_WhatIsGround))
			{
				jump = false;
				crouch = true;
